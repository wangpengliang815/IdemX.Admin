...
/** 伙歌行实名注册及入驻协议（正文见 user-agreement-content.txt，按空行分段展示） */
  ...
  realName: [
    { required: true, message: '请输入真实姓名' },
    {
      validator: (_rule: any, value: string) => {
        if (!value) return Promise.resolve();
        if (value.length < 2) {
          return Promise.reject(new Error('真实姓名至少需要2个字符'));
        }
        if (!/^[\u4E00-\u9FA5]+$/.test(value)) {
          return Promise.reject(new Error('真实姓名只能包含中文'));
        }
        return Promise.resolve();
      },
    },
  ...
  agreePolicy: [
    {
      validator: (_rule: any, value: boolean) => {
        if (!value) {
          return Promise.reject(new Error('请阅读并同意《伙歌行实名注册及入驻协议》'));
        }
        return Promise.resolve();
      },
    },
...
<template>
  <div class="register-page">
    <div class="register-form-wrapper">
      ...
      <Form ref="formRef" :model="formData" :rules="rules" class="register-form" validate-trigger="">
        ...
        <Form.Item name="realName">
          <Input v-model:value="formData.realName" size="large" placeholder="请输入真实姓名（最少2个字符）" autocomplete="name">
            <template #prefix>
              <span class="input-icon">👨</span>
            </template>
          </Input>
        ...
        <Form.Item name="agreePolicy">
          <Checkbox v-model:checked="formData.agreePolicy">
            <span class="agreement-text">
              我已阅读并同意
              <a class="agreement-link" href="" @click="openUserAgreement">《伙歌行实名注册及入驻协议》</a>
            </span>
          </Checkbox>
        </Form.Item>
    ...
    <Modal v-model:open="agreementVisible" title="伙歌行实名注册及入驻协议" :footer="null" width="960px" :mask-closable="true">
