...
/**
 * 获取所有菜单并转换为Vben路由格式
 * 在动态菜单前注入与 core 静态路由一致的默认首页（合约广场），保证侧栏与高亮与 `defaultHomePath` 一致
 * @returns Vben路由配置数组
 */
export async function getAllMenusApi(roleCode: 'admin' | 'buyer' | 'cp') {
  const response = await getMenusApi(roleCode);
  if (response.code !== 0) {
    throw new Error(response.msg);
  }
  const processedVbenMenus = transformMenuData(response.data!);
  ...
  return [plazaMenu, ...processedVbenMenus];
}
