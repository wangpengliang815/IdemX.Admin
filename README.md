...
| 🛡️ 权限模型 | **RBAC**（用户 → 角色 → 菜单），无「用户类型」字段 |
| 🗄️ 数据库 | **SQL 脚本**建表与种子（`schema.sql` + `seed.sql`），不做 CodeFirst 自动迁移 |
...
| 层 | 技术 |
|---|---|
| ⚙️ 后端 | .NET 8、SqlSugar、JWT、Autofac、AutoMapper |
